...
namespace OnlineStore.Pages.Admin
{
    ...
    public class IndexModel : PageModel
    {
        ...
        {
            // Сначала пробуем взять параметр section из query (href="?section=products")
            var sectionQuery = Request.Query["section"].FirstOrDefault();
            if (!string.IsNullOrEmpty(sectionQuery))
            {
                ActiveSection = sectionQuery;
            }
            else
            {
                // Если свойства ActiveSection не передали, оставляем dashboard по умолчанию
        ...
        private async Task LoadDashboardData()
        {
            ...
            // Aggregate on client side because SQLite provider does not support Sum over decimal
            var amounts = await _context.Orders
                .Select(o => o.TotalAmount)
                .ToListAsync(); // materialize decimals into memory

            TotalRevenue = amounts.Any() ? amounts.Sum() : 0m;
